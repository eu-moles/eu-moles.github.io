(() => {
  ...
  const voteBadge = (position) => {
    ...
    badge.setAttribute('aria-label', `This Member voted ${label}`);
  ...
  const voteExplainer = (motion) => {
    ...
    const id = `mep-vote-explainer-${String(motion.id || '').replace(/[^a-zA-Z0-9_-]/g, '-')}`;
    const explainer = make('span', 'motion-vote-explainer');
    const toggle = make('button', 'motion-vote-explainer__toggle');
    toggle.type = 'button';
    toggle.dataset.voteExplainerToggle = '';
    toggle.setAttribute('aria-expanded', 'false');
    toggle.setAttribute('aria-controls', id);
    toggle.setAttribute('aria-label', 'What this vote means?');
    const toggleIcon = make('i', 'fa-solid fa-circle-info');
    toggleIcon.setAttribute('aria-hidden', 'true');
    toggle.append(toggleIcon, make('span', 'motion-vote-explainer__label', 'What this means?'));

    const popup = make('span', 'motion-vote-explainer__popup');
    popup.id = id;
    popup.setAttribute('role', 'dialog');
    popup.setAttribute('aria-label', 'Plain-language guide');

    const header = make('span', 'motion-vote-explainer__mobile-header');
    const headerCopy = make('span');
    headerCopy.append(
      make('span', 'motion-vote-explainer__mobile-eyebrow', 'Plain-language guide'),
      make('span', 'motion-vote-explainer__mobile-title', motion.displayTitle || motion.title || 'Vote details'),
    );
    const close = make('button', 'motion-vote-explainer__close');
    close.type = 'button';
    close.dataset.voteExplainerClose = '';
    close.setAttribute('aria-label', 'Close plain-language guide');
    const closeIcon = make('i', 'fa-solid fa-xmark');
    closeIcon.setAttribute('aria-hidden', 'true');
    close.append(closeIcon);
    header.append(headerCopy, close);

    const note = make('span', 'motion-vote-explainer__note');
    const noteIcon = make('i', 'fa-solid fa-robot');
    noteIcon.setAttribute('aria-hidden', 'true');
    note.append(noteIcon, document.createTextNode('This plain-language guide and its assessment were generated by AI from official European Parliament records.'));

    const section = (modifier, icon, heading, text) => {
      const element = make('span', 'motion-vote-explainer__section');
      const marker = make('i', `motion-vote-explainer__section-icon motion-vote-explainer__section-icon--${modifier} fa-solid ${icon}`);
      marker.setAttribute('aria-hidden', 'true');
      const content = make('span', 'motion-vote-explainer__section-content');
      content.append(
        make('span', 'motion-vote-explainer__eyebrow', heading),
        make('span', 'motion-vote-explainer__text', explainerText(text)),
      );
    ...
    popup.append(
      header,
      note,
      section('description', 'fa-file-lines', 'Description', description),
      section('yes', 'fa-check', 'What a Yes vote means', motion.explainerYesVote),
      section('russia', 'fa-triangle-exclamation', 'Does it benefit Russia?', motion.explainerRussia),
    );
    explainer.append(toggle, popup);
    return explainer;
  };

  const russiaAlert = (motion) => {
    const assessment = explainerText(motion.explainerRussia);
    if (assessment.startsWith('Potentially:')) {
      const alert = make('span', 'motion-subvote-russia-alert');
      alert.setAttribute('role', 'img');
      alert.setAttribute('aria-label', 'Potential benefit to Russia');
      alert.title = 'Potential benefit to Russia';
      const marker = make('i', 'fa-solid fa-triangle-exclamation');
      marker.setAttribute('aria-hidden', 'true');
      alert.append(marker);
      return alert;
    }
    if (assessment === 'Official amendment wording is unavailable, so Russia impact cannot be assessed.') {
      const unknown = make('span', 'motion-subvote-russia-alert motion-subvote-russia-alert--unknown');
      unknown.setAttribute('role', 'img');
      unknown.setAttribute('aria-label', 'Russia impact cannot be assessed');
      unknown.title = 'Russia impact cannot be assessed';
      const marker = make('i', 'fa-solid fa-circle-question');
      marker.setAttribute('aria-hidden', 'true');
          ...
          if (siblingSubvotes.length) {
            ...
            siblingSubvotes.forEach((subvote) => {
              ...
              if (outcome === 'ADOPTED' || outcome === 'REJECTED') {
                ...
                status.setAttribute('aria-label', outcome === 'ADOPTED' ? 'Adopted' : 'Rejected');
                status.title = status.getAttribute('aria-label');
                const statusIcon = make('i', `fa-solid ${outcome === 'ADOPTED' ? 'fa-thumbs-up' : 'fa-thumbs-down'}`);
              ...
              const titleGroup = make('span', 'motion-subvote-title');
              ...
              titleGroup.append(subvoteTitle);
              const alert = russiaAlert(subvote);
              if (alert) titleGroup.append(alert);
              primary.append(titleGroup);
              const explainer = voteExplainer(subvote);
              if (explainer) {
                const explainerRow = make('span', 'motion-vote-explainer-row');
                explainerRow.append(explainer);
                primary.append(explainerRow);
              }
